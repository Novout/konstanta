...
export const CreateAltarButton = (app, player, item, resources) => {
  const main = KContainer(app.stage, { x: player.x - player.width / 2, y: player.y - player.height / 2 }, [['id_parent', item.id]]);

  const container = KGraphics(main, { fill: 0x66BD99, rectangle: [0, 0, 256, 64], filters: [UIAlpha().alpha_main] });

  const button = KText('Ativar Altar', container, {
    fontFamily: "Poppins",
    fontSize: 36,
    fontWeight: "bold"
  }, {
  ...
  button.on('click', () => {
    ...
    arr.forEach(item => {
      ...
      const item_container = KGraphics(choice_skills, { fake: true, rectangle: [0, 0, 128, 256] });
      ...
      const item_title = KText(item.title, item_container, 
        { fontFamily: 'Poppins', fontSize: 24, fontWeight: 'bold' },
        { positional: { y: item_sprite.height + 8, absolute: true }
      });

      const item_description = KText(item.description, item_container, 
        { fontFamily: 'Poppins', fontSize: 16 }, 
        { positional: { y: item_title.y + 40, absolute: true }
      });

      const item_button_text = KText('Escolher', item_container, {
        fontFamily: "Poppins",
        fontSize: 18,
        fontWeight: "bold"
      }, {
  ...
  const choice_skills = KGraphics(main, { fake: true, rectangle: [0, 0, 512, 512] });
  choice_skills.visible = false;

  const item_container = KGraphics(choice_skills, { fake: true, rectangle: [0, 0, 128, 256] });

  const choice_skills_fake = KGraphics(choice_skills, { fill: 0x66BD99, rectangle: [0, 0, 512, 512], filters: [InterfaceGlow(), UIAlpha().alpha_bar] });
